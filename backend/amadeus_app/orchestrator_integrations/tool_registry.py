"""Unified tool registry for orchestrator integrations.

Merges three sources of tools into a single OpenAI-compatible function
calling schema:

1. Builtin tools registered in :mod:`amadeus_app.builtin_tool_registry`.
...
The registry provides one process-wide view over builtin tools, MCP tools, and
skill allowlists. Task runners and chat/orchestrator flows take filtered
snapshots from this shared source instead of keeping separate registries.
"""
...
@dataclass
class UnifiedTool:
    """A tool surfaced to the orchestrator runtime.
