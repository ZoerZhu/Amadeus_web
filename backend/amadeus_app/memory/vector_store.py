...
def embedding_dim() -> int:
    return int(os.getenv("AMADEUS_MEMORY_EMBEDDING_DIM", str(EMBEDDING_DIM)))
...
def ensure_vec_tables(conn: sqlite3.Connection) -> None:
    ...
    """
    dim = embedding_dim()
    _drop_vec_tables_if_dimension_changed(conn, dim)
    conn.execute(
        f"""
...
def _drop_vec_tables_if_dimension_changed(conn: sqlite3.Connection, expected_dim: int) -> None:
    """sqlite-vec 虚拟表维度不可 ALTER，维度变化时重建空表。"""
    expected_tables = ("memory_node_vectors", "memory_leaf_vectors")
    mismatch = False
    for table in expected_tables:
        ...
        match = re.search(r"embedding\s+FLOAT\[(\d+)\]", sql, re.IGNORECASE)
        current_dim = int(match.group(1)) if match else None
        if current_dim != expected_dim:
            mismatch = True
            _log.warning(
                "memory vector table dimension changed: table=%s current=%s expected=%d; rebuilding vector tables",
                table,
                current_dim,
                expected_dim,
            )
