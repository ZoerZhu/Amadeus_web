...
async def create_agent_task(request: AgentTaskCreateRequest) -> dict[str, Any]:
    """Create an agent task and run it in the background.

    Use GET /api/agent/tasks/{id}/events to subscribe to progress.
    """
    storage = require_storage()
    agent_settings = await _get_agent_settings_async()
    if not agent_settings.enabled:
    ...
    await agent_task_hub.attach_background_task(task.task_id, background)
    return {"ok": True, "taskId": task.task_id, "id": task.task_id}
